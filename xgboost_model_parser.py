            ...
            if line.strip():  
                ...
                if (newLine != oldLine):
                    ...
                    for k in feature_res: # 这一行的feature
                    	...
                    	line_feature_dict[ feature_id ] = 1 # 把当前样本已经有的feature放入dict中作为key
                    str_to_add = " "
                    ...
                    for tree_id in range(0, len(list_from_dict) - 1 ):
                    	...
                    	if new_feature_id > 0:                  	
                    		...
                    		line_feature_dict[ new_feature_id ] = 1

                    sorted_line_feature_dict = sorted(line_feature_dict.items(), key=lambda d:d[0]) # new_feature_id 根据key排序
                    str_to_print = ""
                    for key in sorted_line_feature_dict:
                    	str_to_print = str_to_print + str(key[0]) + ":1 "
                    str_to_print = label + " " + str_to_print
